...
class CookieJarSql {
  ...
  List<Cookie> _loadWithDomain(String domain) {
    ...
    return rows.map(_cookieFromRow).nonNulls.toList();
  }

  Cookie? _cookieFromRow(Row row) {
    final name = row["name"];
    ...
    if (name is! String ||
      ...
      Log.warning("Network", "Skipping malformed cookie row for $domain");
      return null;
    }
  ...
  String loadForRequestCookieHeader(Uri uri) {
    ...
    for (var cookie in cookies) {
      final previous = map[cookie.name];
      if (previous != null) {
        final domain = cookie.domain ?? '';
        final previousDomain = previous.domain ?? '';
        if (!domain.startsWith('.') && previousDomain.startsWith('.')) {
          map[cookie.name] = cookie;
        } else if (domain.length > previousDomain.length) {
          map[cookie.name] = cookie;
        }
      } else {
        map[cookie.name] = cookie;
