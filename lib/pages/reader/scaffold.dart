...
class _ReaderScaffoldState extends State<_ReaderScaffold> {
  ...
  bool _dragListenerRetryScheduled = false;

  void addDragListener() async {
    if (!mounted) return;
    if (_gestureDetectorState == null) {
      if (_dragListenerRetryScheduled) {
        return;
      }
      _dragListenerRetryScheduled = true;
      WidgetsBinding.instance.addPostFrameCallback((_) {
        _dragListenerRetryScheduled = false;
        if (mounted) {
          addDragListener();
        }
      });
